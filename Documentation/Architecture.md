...
Decision: September 8, 2026. The following modules are planned, not implemented.

```text
Native app (private)             amend CLI (public)
          \                      /
              AmendCore (public)
              /              \
     AmendDomain          Adapter interfaces
                         /        |        \
                  Apple APIs  engine packs  AI providers
```

`AmendDomain` owns immutable inspection, capabilities, requests, plans, events, results and structured failures. It has no UI, commerce or network dependency.

`AmendCore` owns job coordination, resource policies, cancellation, safe temporary outputs and verification/finalization. Adapters own engine-specific options, execution, progress and validation. An explicit provider adapter may perform a user-authorized network operation; ordinary local conversion never silently switches to it.
