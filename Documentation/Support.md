...
| Media with the verified FFmpeg pack | MP4/MOV H.264 video; M4A/AAC, WAV/16-bit PCM, FLAC audio; [MP3](MP3-output.md) with pack .2 or newer; audio extraction; remux when compatible; bounded target-size attempts | Maximum six-hour duration. Video outputs require one video/at most one audio track, square pixels and supported SDR properties. Explicit resizing for odd dimensions. Audio extraction can ignore unsupported video properties; multi-audio selection is not implemented |
| Media trim with the verified FFmpeg pack | Exact frame/sample trimming; eligible H.264/AAC packet copy with measured snapped bounds; explicit audio-track selection for trim | Constant-rate even-sized SDR picture; MP4-family fast copy without reordered picture frames. Packet, duration and fidelity bounds apply; see [media trimming](Media-trimming.md). MP3, VFR trim and general stream preservation remain unavailable |
...
| Unencrypted PDF with the verified qpdf pack and native worker | [Embedded JPEG/PNG extraction](PDF-embedded-images.md) from selected page resources and nested Forms | Unique source/object/generation provenance; eligible original JPEG bytes or exact 8-bit DeviceRGB/Gray samples with supported soft masks. Explicit unsupported candidates; no page raster fallback. At most 1000 images and 512 MiB cumulative decoded/output budget |
